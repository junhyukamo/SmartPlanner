...
export default function App() {
  ...
  const [isMobile, setIsMobile] = useState(false); // 💡 모바일 화면 감지 상태
...
plans 배열은 무조건 12개의 문자열로 구성. 요청되지 않은 달은 빈 문자열("")로 두세요.`,
  ...
  // 💡 모바일 월간 탭의 블록 렌더링 설정 (모바일은 4블록, PC는 2블록)
  ...
  return (
    <>
          ...
          {view === 'PLANNER' && (
            <div className="flex flex-col h-full w-full relative">
              ...
              <main className={`flex-1 min-h-0 w-full mx-auto relative text-center flex flex-col ${activeTab === 'WEEKLY' ? 'p-1 md:p-2' : 'p-2 md:p-6 pb-24'} overflow-y-auto custom-scrollbar`}>
                          ...
                          <table className="w-full h-full min-h-full text-center border-collapse table-fixed weekly-mobile-table">
                            <thead className="z-20 shadow-sm border-b-2 border-slate-200 text-slate-800 bg-slate-50 sticky top-0">
                              <tr style={{ height: '30px' }}>
                                <th className={`border-r border-slate-200 uppercase font-black z-20 align-middle transition-colors duration-200 time-col ${wBounds ? 'bg-indigo-100 text-indigo-700' : 'text-slate-400'}`} style={{ width: isMobile ? '30px' : '45px', fontSize: `${Math.max(6, displayFontSize - 2)}px` }}>
                  ...
                  <div className="animate-fade-in flex flex-col gap-6 text-center w-full overflow-x-hidden">
                    <div className="p-2 md:p-6 rounded-3xl border border-slate-200 bg-white shadow-sm w-full text-center">
                      ...
                      {monthlyBlocks.map((blockIdx) => {
                        ...
                        // 💡 교재 ID 중복 방지 로직 (모바일의 1주차와 2주차는 같은 교재를 공유함)
                        ...
                        return (
                          <div key={blockIdx} className={`w-full relative select-none ${isMobile ? 'pb-4' : 'overflow-x-auto custom-scrollbar pb-3'}`} onMouseLeave={handleMouseUp}>
                            {isMobile && <div className="text-left font-black text-indigo-600 text-[10px] mb-1 pl-1">{blockIdx + 1}주차</div>}
                            <div className={isMobile ? "w-full" : "min-w-[800px] md:min-w-full"}>
                              <table className={`w-full border-collapse ${isMobile ? 'mb-2 text-[8px] monthly-mobile-table' : 'mb-4 md:mb-10 text-[9px] md:text-[11px]'} table-fixed text-center align-middle`}>
                                <thead>
                                ...
                                <tbody>
                                  <tr className="bg-white text-center">
                                    ...
                                    {chunk.map((d, i) => {
                                      // 💡 모바일 4블록을 위한 cIdx (키보드 네비게이션 매핑)
                                      const cIdx = isMobile
                                        ? (blockIdx < 2 ? blockIdx * 7 + i + 1 : (blockIdx - 2) * 7 + i + 16)
                                        : (blockIdx === 0 ? i + 1 : i + 16);
                                      const rIdx = 0;
                                  ...
                                  {flatRows.map((rowInfo) => {
                                    ...
                                    // 💡 모바일 4블록을 위한 tbCIdx 매핑
                                    const tbCIdx = isMobile
                                      ? (blockIdx < 2 ? 0 : 15)
                                      : (blockIdx === 0 ? 0 : 15);
                                        ...
                                        >
                                          ...
                                          <div className="w-full h-full flex flex-col justify-center items-center p-0 text-center min-h-[36px] md:min-h-[50px] relative">
                                            <textarea 
                                              ...
                                              placeholder={tbIdx === 0 && rowSpan === 1 ? (isMobile ? "교재" : "교재 입력\n(Shift+Enter 행 추가)") : ""}
                                              onCompositionStart={(e) => {
                ...
                {activeTab === 'YEARLY' && (
                  <div className="animate-fade-in grid grid-cols-3 md:grid-cols-6 gap-2 md:gap-4 text-center yearly-mobile-grid">
                    {yearlyPlan.map((plan, idx) => (
                      <div key={idx} className="p-1 md:p-6 rounded-2xl md:rounded-3xl border border-slate-200 bg-white shadow-sm transition-all hover:shadow-md text-center">
                        ...
                        <textarea 
                          ...
                          className="w-full p-1.5 md:p-4 rounded-lg md:rounded-xl border border-slate-100 outline-none focus:border-indigo-500 transition-all font-bold resize-none text-center overflow-hidden bg-transparent auto-resize min-h-[60px] md:min-h-[80px] break-words whitespace-pre-wrap" 
                        />
                      </div>
                    ))}
                  </div>
                ...
                {/* 💡 [학생 정보 탭 렌더링 구현] */}
                {activeTab === 'INFO' && (
                  <div className="animate-fade-in flex flex-col gap-4 w-full text-center overflow-x-hidden p-2 md:p-4 bg-white rounded-3xl border border-slate-200 shadow-sm info-mobile-fit">
                    {/* 상단: 웹사이트 및 선택과목 */}
                    <div className="w-full overflow-x-auto custom-scrollbar">
                      <table className="w-full border-collapse text-center table-fixed md:min-w-[700px] min-w-full info-mobile-table">
                        <thead>
                        ...
                        <tbody>
                          {studentInfo.websites.map((row, rIdx) => (
                            <tr key={rIdx} className="hover:bg-slate-50 transition-colors">
                              ...
                              {rIdx === 0 && renderInfoCell('electives', 0, 0, studentInfo.electives[0], { rowSpan: studentInfo.websites.length, extraStyle: { fontSize: `${displayFontSize}px`, fontWeight: 700, minHeight: '80px' } })}
                              {rIdx === 0 && renderInfoCell('electives', 0, 1, studentInfo.electives[1], { rowSpan: studentInfo.websites.length, extraStyle: { fontSize: `${displayFontSize}px`, fontWeight: 700, minHeight: '80px' } })}
                            </tr>
                          ))}
                        </tbody>
                      </table>
                    ...
                    <div className="flex flex-col xl:flex-row gap-4 w-full mt-2">
                      ...
                      <div className="flex-1 w-full overflow-x-auto custom-scrollbar">
                        <div className="bg-slate-100 border border-slate-300 p-0 font-black text-center mb-0 border-b-0 rounded-t-xl overflow-hidden shadow-sm">
                          {renderInfoCell('midHeaders', 0, null, studentInfo.midHeaders[0], { tag: 'div', extraClass: 'w-full bg-slate-100 !border-none', extraStyle: { fontSize: `${Math.max(10, displayFontSize + 2)}px`, fontWeight: 900 } })}
                        </div>
                        <table className="w-full md:min-w-[600px] min-w-full border-collapse text-center table-fixed bg-white shadow-sm rounded-b-xl overflow-hidden info-mobile-table">
                          <tbody>
                      ...
                      <div className="flex-1 w-full overflow-x-auto custom-scrollbar">
                        <div className="bg-slate-100 border border-slate-300 p-0 font-black text-center mb-0 border-b-0 rounded-t-xl overflow-hidden shadow-sm">
                          {renderInfoCell('midHeaders', 1, null, studentInfo.midHeaders[1], { tag: 'div', extraClass: 'w-full bg-slate-100 !border-none', extraStyle: { fontSize: `${Math.max(10, displayFontSize + 2)}px`, fontWeight: 900 } })}
                        </div>
                        <table className="w-full md:min-w-[500px] min-w-full border-collapse text-center table-fixed bg-white shadow-sm rounded-b-xl overflow-hidden info-mobile-table">
                          <tbody>
      ...
      <style dangerouslySetInnerHTML={{ __html: `
        ...
        /* 📱 [모바일 한정] 가로 스크롤 방지 및 Print Preview 효과를 위한 강제 리사이징 */
        @media (max-width: 767px) {
          /* 주간 탭 */
          .weekly-mobile-table th { padding: 2px 0 !important; }
          .weekly-mobile-table td { padding: 0 !important; }
          .weekly-mobile-table .time-col { width: 30px !important; }
          .weekly-mobile-table .time-col-inner { min-height: 20px !important; padding: 0 !important;}
          .weekly-mobile-table textarea { line-height: 1.1 !important; padding: 1px !important; min-height: 20px !important; }

          /* 월간 탭 */
          .monthly-mobile-table th, .monthly-mobile-table td { padding: 1px !important; }
          .monthly-mobile-table textarea { line-height: 1.1 !important; padding: 1px !important; }
          .monthly-summary-table { min-width: 100% !important; width: 100% !important; }
          .monthly-summary-table th, .monthly-summary-table td { padding: 2px !important; word-break: keep-all; }
          
          /* 연간 탭 */
          .yearly-mobile-grid { grid-template-columns: repeat(3, minmax(0, 1fr)) !important; gap: 4px !important; }
          .yearly-mobile-grid textarea { min-height: 60px !important; padding: 4px !important; }

          /* 정보 탭 */
          .info-mobile-table { min-width: 100% !important; width: 100% !important; }
          .info-mobile-table th, .info-mobile-table td { padding: 1px !important; }
          .info-mobile-table textarea { line-height: 1.1 !important; padding: 1px !important; }
          .info-mobile-fit { padding: 8px 4px !important; overflow-x: hidden !important; }
        }
