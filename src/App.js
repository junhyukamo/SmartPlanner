...
// 💡 [에러 해결] Firebase 2차원 배열 저장 금지 제약을 우회하여 안전하게 객체로 포장하는 인코딩 로직
...
// 💡 [에러 해결] 서버에서 데이터를 받아왔을 때 다시 화면에 그릴 수 있도록 2차원 배열로 평탄화(복구)
...
export default function App() {
  ...
  // 💡 [데이터 읽기] 서버에서 가져온 1차원 데이터(포장된 상태)를 다시 2차원 표로 풀어줍니다.
  useEffect(() => {
  ...
  // 💡 [데이터 쓰기] 다차원 배열 저장 오류를 방지하기 위해 저장 직전에 1차원 객체로 포장합니다.
    ...
    const saveData = async () => {
      ...
      try {
        await setDoc(doc(db, 'planners', currentDocId), { 
          ...
          studentInfo: encodeStudentInfo(studentInfo), // 💡 2차원 배열 포장
  ...
  // 💡 [추가됨] 학생 정보 탭 내에서 엔터 치면 셀 추가, 빈칸 백스페이스 삭제 로직
  const handleInfoKeyDown = (e, section, rIdx, cIdx) => {
    ...
    if (e.key === 'Enter') {
        if (e.shiftKey) {
            ...
            saveToHistory();
            // 💡 [오류 수정] 복사할 때 배열의 깊은 복사(Deep Copy)를 수행하여 참조 버그 방지
        ...
        if ((section === 'schoolGrades' || section === 'mockGrades') && studentInfo[section].length > 1) {
            e.preventDefault();
            saveToHistory();
            // 💡 [오류 수정] 깊은 복사로 상태를 완전히 새롭게 구성하여 버그 방지
            const newInfo = { ...studentInfo, [section]: [...studentInfo[section]] };
