  ...
  const handleBlur = (e, id, day, isMonthly, subject, dateKey) => {
    if (e && e.target) {
      ...
      if (!isMonthly && id && day) {
        ...
        // 💡 [A/B/C콤 스마트 이동 로직 (셀 위치는 셀 규격에 맞게 이동, 표기 텍스트는 원본 유지)]
        ...
        if (comboMatch) {
          ...
          isCombo = true;
          targetSpan = 3; // 1시간 30분 = 3칸
          
          if (isWeekend) { // 토,일: A(2:30), B(4:00), C(5:30) 이동
            if (type === 'a') targetId = 14;      // 14:30 시작 위치
            else if (type === 'b') targetId = 17; // 16:00 시작 위치
            else if (type === 'c') targetId = 20; // 17:30 시작 위치
          } else { // 월~금: A(5:30), B(7:00), C(8:30) 이동
            if (type === 'a') targetId = 20;      // 17:30 시작 위치
            else if (type === 'b') targetId = 23; // 19:00 시작 위치
            else if (type === 'c') targetId = 26; // 20:30 시작 위치
          }
        }
...
plans 배열은 무조건 12개의 문자열로 구성. 요청되지 않은 달은 빈 문자열("")로 두세요.` 
  ...
  // 💡 [삭제된 시트 접근 차단 데드엔드 구현]
  if (isNotFound && view === 'PLANNER') {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-slate-50 p-6 text-center animate-fade-in">
        ...
        <h1 className="text-2xl md:text-3xl font-black text-slate-500">삭제된 시트입니다.</h1>
        <p className="text-slate-400 font-medium mt-2">더 이상 접근할 수 없는 플래너입니다.</p>
      </div>
    );
  }
  ...
  return (
    <>
        ...
        <div className={`w-full mx-auto ${(view === 'PLANNER' && activeTab === 'WEEKLY') ? 'flex-1 flex flex-col min-h-0' : ''}`}>
          ...
          {view === 'PLANNER' && (
            <div className="flex flex-col h-full w-full relative">
              {/* 💡 [모바일 헤더 다이어트] 패딩 축소로 공간 확보 */}
              <header className="flex-none px-2 md:px-4 py-1.5 md:py-3 shadow-sm z-30 bg-white border-b border-slate-200 relative">
                <div className="max-w-[98vw] mx-auto flex flex-col md:flex-row justify-between items-center gap-1 md:gap-4">
                  <div className="flex items-center gap-2 w-full md:w-auto justify-between md:justify-start">
                    <div className="flex items-center gap-1.5 md:gap-3">
                      {role === 'teacher' && <button onClick={() => handleSafeBack('TEACHER_DASHBOARD')} className="p-1 md:p-2 rounded-xl hover:bg-slate-100 border border-slate-200"><ChevronLeft className="w-4 h-4 md:w-5 md:h-5" /></button>}
                      <div className="p-1.5 md:p-2.5 rounded-xl shadow-inner bg-gradient-to-br from-indigo-500 to-indigo-700"><BookOpen className="text-white w-3.5 h-3.5 md:w-5 md:h-5" /></div>
                      <div className="font-black text-sm md:text-xl tracking-tight truncate max-w-[150px] sm:max-w-[300px]">{studentName} 플래너</div>
                    </div>
                  </div>
                  <div className="flex items-center gap-1.5 md:gap-3 w-full md:w-auto justify-between md:justify-end">
                    
                    <div className="flex items-center bg-slate-100 rounded md:rounded-lg p-0.5 border border-slate-200 shadow-inner">
                      <button onClick={() => setFontSize(f => Math.max(8, f - 1))} className="px-1.5 py-1 md:px-2 md:py-1.5 hover:bg-white hover:shadow-sm rounded text-slate-600 font-black transition-all flex items-center justify-center"><Minus size={12} className="w-3 h-3 md:w-3.5 md:h-3.5"/></button>
                      <span className="text-[10px] md:text-xs font-black w-4 md:w-6 text-center text-indigo-700 select-none cursor-default">{fontSize}</span>
                      <button onClick={() => setFontSize(f => Math.min(24, f + 1))} className="px-1.5 py-1 md:px-2 md:py-1.5 hover:bg-white hover:shadow-sm rounded text-slate-600 font-black transition-all flex items-center justify-center"><Plus size={12} className="w-3 h-3 md:w-3.5 md:h-3.5"/></button>
                    </div>

                    <div className="flex p-0.5 md:p-1 rounded md:rounded-xl shadow-inner bg-slate-100 flex-1 md:flex-none justify-center">
                      {['WEEKLY', 'MONTHLY', 'YEARLY'].map((tab) => (
                        ...
                        }} className={`flex-1 md:flex-none px-3 md:px-6 py-1 md:py-2 rounded md:rounded-lg text-[10px] md:text-sm font-extrabold transition-all duration-300 ${activeTab === tab ? "bg-white text-indigo-700 shadow-md scale-[1.02]" : "text-slate-400 hover:text-slate-600"}`}>{tab === 'WEEKLY' ? '주간' : tab === 'MONTHLY' ? '월간' : '연간'}</button>
                      ))}
                    </div>
                    {role === 'teacher' && (
              ...
              <main className={`flex-1 min-h-0 w-full mx-auto relative text-center flex flex-col ${activeTab === 'WEEKLY' ? 'p-0.5 md:p-2' : 'p-1 md:p-4 pb-24'} overflow-y-auto custom-scrollbar`}>
                
                {activeTab === 'WEEKLY' && (
                  <div className="animate-fade-in flex flex-col text-center h-full">
                    <div className="flex-1 flex flex-col min-h-0">
                      {/* 💡 [테이블 그림자 없애고 인쇄 스타일 적용] */}
                      <div className="flex flex-col h-full relative z-30 bg-white md:rounded-xl md:shadow-sm md:border border-slate-200">
                        
                        <div className="flex flex-wrap items-center justify-between gap-1 mb-1 md:mb-2 flex-shrink-0 px-1 md:px-2 py-1 md:pt-2 relative z-40 bg-slate-50 md:bg-transparent border-b border-slate-200 md:border-0">
                          <div className="flex flex-wrap items-center gap-1 md:gap-2">
                            {dDay ? (
                              <div className="flex items-center gap-1 md:gap-2 px-2 md:px-3 py-1 md:py-1.5 bg-slate-800 text-white rounded shadow-sm text-[10px] md:text-xs text-center tracking-tight">
                                ...
                                <button onClick={() => setDDay(null)} className="hover:text-red-200 p-0.5 ml-0.5 md:ml-1"><X className="w-3 h-3" /></button>
                              </div>
                            ) : (
                              <div className="flex items-center gap-1 p-0.5 md:p-1 rounded border border-slate-200 bg-white shadow-sm justify-center">
                                <input type="text" placeholder="제목" className="w-12 md:w-28 p-0.5 md:p-1 text-[10px] md:text-xs rounded outline-none font-bold bg-slate-50 border border-slate-100 focus:border-indigo-500 text-center" value={dDayInput.title} onChange={(e) => setDDayInput({ ...dDayInput, title: e.target.value })}/>
                                <input type="date" className="w-[85px] md:w-24 p-0.5 md:p-1 text-[10px] md:text-xs rounded outline-none bg-slate-50 border border-slate-100 focus:border-indigo-500 text-center" value={dDayInput.date} onChange={(e) => setDDayInput({ ...dDayInput, date: e.target.value })}/>
                                <button onClick={() => { if (dDayInput.title) { setDDay(dDayInput); setDDayInput({ title: '', date: '' }); saveToHistory(); } }} className="px-2 py-0.5 md:py-1 rounded text-[10px] md:text-xs font-bold transition-colors bg-slate-800 hover:bg-slate-900 text-white">설정</button>
                              </div>
                            )}
                          </div>
                          
                          {/* 💡 [모바일에서 텍스트는 숨기고 꽉 찬 아이콘 버튼으로 변경] */}
                          <div className="flex flex-wrap items-center justify-end gap-1 ml-auto relative">
                            
                            <button onClick={() => { setPrintConfig(prev => ({ ...prev, scope: 'all' })); setShowPrintModal(true); }} className="hidden sm:flex items-center justify-center gap-1 px-3 py-1.5 rounded text-xs font-bold transition-colors border border-slate-300 bg-white text-slate-700 hover:bg-slate-50">
                              <Printer className="w-3 h-3" /> <span>인쇄</span>
                            </button>

                            <button onClick={() => setShowColorModal(!showColorModal)} className={`flex items-center justify-center w-7 h-7 md:w-auto md:h-auto md:px-3 md:py-1.5 rounded text-[10px] md:text-xs font-bold transition-all border ${showColorModal ? 'bg-indigo-50 border-indigo-300 text-indigo-700' : 'bg-white border-slate-300 text-slate-700 hover:bg-slate-50'}`}>
                              <Palette className="w-3.5 h-3.5 md:w-3 md:h-3" /> <span className="hidden md:inline ml-1">색상</span>
                            </button>

                            <div className="h-4 w-px mx-0.5 bg-slate-300 hidden md:block"></div>

                            {isWMulti ? <button onClick={mergeCells} className="flex items-center justify-center w-7 h-7 md:w-auto md:h-auto md:px-3 md:py-1.5 rounded bg-indigo-600 text-white hover:bg-indigo-700 font-bold text-xs"><Merge className="w-3.5 h-3.5 md:w-3 md:h-3" /> <span className="hidden md:inline ml-1">병합</span></button> : <div className="flex items-center justify-center w-7 h-7 md:w-auto md:h-auto md:px-3 md:py-1.5 rounded text-[10px] font-medium border border-dashed border-slate-300 text-slate-400 bg-slate-50 select-none"><MousePointer2 className="w-3.5 h-3.5 md:w-3 md:h-3" /> <span className="hidden md:inline ml-1">드래그</span></div>}
                            <button onClick={unmergeCells} className="flex items-center justify-center w-7 h-7 md:w-auto md:h-auto md:px-3 md:py-1.5 rounded text-[10px] md:text-xs font-bold border border-slate-300 text-slate-700 hover:bg-slate-50 bg-white"><Split className="w-3.5 h-3.5 md:w-3 md:h-3" /> <span className="hidden md:inline ml-1">분할</span></button>
                            <button onClick={() => setShowResetConfirm(true)} className="flex items-center justify-center w-7 h-7 md:w-auto md:h-auto md:px-3 md:py-1.5 rounded text-[10px] md:text-xs font-bold bg-red-50 text-red-600 border border-red-200 hover:bg-red-100 ml-0.5"><Trash2 className="w-3.5 h-3.5 md:w-3 md:h-3" /> <span className="hidden md:inline ml-1">초기화</span></button>
                          </div>
                        </div>
                        
                        {/* 💡 [주간 시트 한눈에 들어오게 모바일 가로 스크롤 제거 & 인쇄 감성 플랫 디자인] */}
                        <div className="w-full flex-1 relative select-none bg-white text-center overflow-y-auto overflow-x-hidden custom-scrollbar z-10" onMouseLeave={handleMouseUp}>
                          <table className="w-full h-full min-h-full text-center border-collapse table-fixed bg-white border-t border-slate-300">
                            <thead className="z-20 bg-slate-50 sticky top-0 border-b border-slate-300">
                              <tr style={{ height: '24px' }}>
                                <th className={`w-8 sm:w-12 font-black z-20 align-middle bg-slate-100 border-r border-slate-300 ${wBounds ? 'text-indigo-700' : 'text-slate-500'}`} style={{ fontSize: `calc(${Math.max(8, fontSize - 2)}px * var(--mobile-scale, 1))` }}>
                                  <span className="sm:hidden">시</span>
                                ...
                                {DAYS.map((d, i) => {
                                  ...
                                  let defaultTextColor = (d === 'sat') ? 'text-blue-600' : (d === 'sun') ? 'text-red-600' : 'text-slate-700';
                                  let textColor = isColSelected ? 'text-indigo-800' : defaultTextColor;
                                  let bgColor = isColSelected ? 'bg-indigo-100/50' : 'bg-transparent';
                                  return (
                                    <th key={d} className={`font-black z-20 align-middle py-0.5 px-0 border-r border-slate-300 ${textColor} ${bgColor}`} style={{ fontSize: `calc(${Math.max(9, fontSize - 1)}px * var(--mobile-scale, 1))` }}>
                            ...
                            <tbody>
                              {timetable.map((row) => {
                                ...
                                const timeBgClass = isRowSelected ? "bg-indigo-50" : "bg-slate-50";
                                ...
                                return (
                                  <tr key={row.id} className="group text-center h-[1%]">
                                    <td className={`p-0 align-middle select-none border-r border-b border-slate-300 ${timeBgClass}`}>
                                      <div className={`flex flex-col items-center justify-center w-full h-full min-h-[20px] sm:min-h-[28px] ${timeTextClass}`} style={{ fontSize: `calc(${Math.max(8, fontSize - 3)}px * var(--mobile-scale, 1))`, letterSpacing: '-0.5px' }}>
                                        <span>{row.time}</span>
                                      </div>
                                    </td>
                                    {DAYS.map((day) => {
                                      ...
                                      const bgColor = isSelected ? 'rgba(224, 231, 255, 0.6)' : keywordColor ? keywordColor : 'transparent';
                                      
                                      return (
                                        <td 
                                          key={day} 
                                          className={`p-0 relative align-top transition-colors duration-200 border-r border-b border-slate-300 ${isSelected ? 'ring-2 ring-indigo-500 ring-inset z-10' : ''} hover:bg-indigo-50/30 ${isEditingThis ? 'cursor-text' : 'cursor-cell'}`} 
                                        ...
                                        >
                                          <div className="w-full h-full flex flex-col items-center justify-center p-0 sm:p-0.5 text-center min-h-[20px] sm:min-h-[28px]">
                                            <textarea 
                                              ...
                                              style={{
                                                ...
                                                fontSize: `calc(${fontSize}px * var(--mobile-scale, 1))`,
                                                lineHeight: '1.15'
                                              }}
                                              ...
                                              className={`w-full p-0 m-0 text-center bg-transparent resize-none outline-none overflow-hidden font-bold align-middle auto-resize break-all sm:break-words whitespace-pre-wrap ${(isActiveThis && !isEditingThis) ? 'select-none' : ''}`} 
                                              rows={1}
                                            />
                                          </div>
                                        </td>
                ...
                {activeTab === 'MONTHLY' && (
                  <div className="animate-fade-in flex flex-col gap-4 md:gap-6 text-center w-full overflow-x-hidden">
                    <div className="p-0.5 md:p-6 w-full text-center">
                      <div className="flex items-center justify-between mb-2 md:mb-6 px-1 md:px-2 text-center mt-1 md:mt-0">
                        <div className="flex items-center gap-2 md:gap-4 text-center">
                          <div className="flex gap-1 md:gap-2 text-center">
                            <button onClick={handlePrev4Weeks} className="p-1 md:p-2 rounded bg-white border border-slate-200 hover:bg-slate-50 transition-colors text-center flex items-center justify-center shadow-sm"><ChevronLeft size={16} className="md:w-5 md:h-5"/></button>
                            <button onClick={handleNext4Weeks} className="p-1 md:p-2 rounded bg-white border border-slate-200 hover:bg-slate-50 transition-colors text-center flex items-center justify-center shadow-sm"><ChevronRight size={16} className="md:w-5 md:h-5"/></button>
                          </div>
                          <div className="font-extrabold text-slate-700 text-[10px] md:text-sm whitespace-nowrap">
                        ...
                        <div className="flex gap-1.5 md:gap-2 text-center">
                          <button onClick={() => { const name = prompt("추가할 과목명을 입력하세요\n(예: 국어, 수학, 영어)"); if(name) name.split(',').forEach(n => { if (n.trim()) addSubjectRow(n.trim()); }); }} className="flex items-center gap-1 px-2 md:px-5 py-1.5 md:py-2.5 bg-indigo-600 text-white rounded font-extrabold text-[10px] md:text-sm hover:bg-indigo-700 shadow-sm transition-all text-center"><Plus size={12} className="md:w-4 md:h-4"/> <span className="hidden sm:inline">과목 추가</span></button>
                          <button onClick={() => setShowResetConfirm(true)} className="flex items-center gap-1 px-2 md:px-5 py-1.5 md:py-2.5 bg-red-50 text-red-600 border border-red-200 rounded font-extrabold text-[10px] md:text-sm hover:bg-red-100 transition-all text-center"><Trash2 size={12} className="md:w-4 md:h-4"/> <span className="hidden sm:inline">일정 초기화</span></button>
                        </div>
                      </div>
                      ...
                      {[0, 1].map((blockIdx) => {
                        ...
                        return (
                          // 💡 [월간 시트 틀 고정 및 가로 스와이프 기능] 모바일에서 찌그러지지 않고 넘겨서 볼 수 있음
                          <div key={blockIdx} className="w-full relative select-none overflow-x-auto custom-scrollbar pb-3" onMouseLeave={handleMouseUp}>
                            <div className="min-w-[700px] md:min-w-full">
                              <table className="w-full border-collapse mb-2 md:mb-6 text-[9px] md:text-[11px] table-fixed text-center align-middle bg-white shadow-sm border border-slate-300">
                                <thead>
                                  <tr>
                                    <th className="w-[45px] md:w-[60px] py-1 md:py-2 text-center font-black align-middle bg-slate-100 text-slate-700 sticky left-0 z-[25] border-r border-b border-slate-300" rowSpan={2} style={{ fontSize: `calc(${Math.max(9, fontSize - 1)}px * var(--mobile-scale, 1))` }}>과목</th>
                                    <th className="w-[55px] md:w-[70px] py-1 md:py-2 text-center font-black align-middle bg-slate-100 text-slate-700 sticky left-[45px] md:left-[60px] z-[25] border-r border-b border-slate-300 shadow-[1px_0_0_#cbd5e1]" rowSpan={2} style={{ fontSize: `calc(${Math.max(9, fontSize - 1)}px * var(--mobile-scale, 1))` }}>교재</th>
                                    {chunk.map((d, i) => {
                                      let textColor = d.isSat ? 'text-blue-600' : d.isWeekend ? 'text-red-600' : 'text-slate-700';
                                      return <th key={i} className={`py-0.5 md:py-1 font-black text-center align-middle bg-slate-50 border-r border-b border-slate-300 ${textColor}`} style={{ fontSize: `calc(${Math.max(9, fontSize - 1)}px * var(--mobile-scale, 1))` }}>{d.day}</th>;
                                    })}
                                  </tr>
                                  <tr>
                                    {chunk.map((d, i) => {
                                       let textColor = d.isSat ? 'text-blue-600' : d.isWeekend ? 'text-red-600' : 'text-slate-700';
                                       return <th key={i} className={`py-0.5 md:py-1 font-bold text-center align-middle break-keep bg-slate-50 border-r border-b border-slate-300 ${textColor}`} style={{ fontSize: `calc(${Math.max(8, fontSize - 2)}px * var(--mobile-scale, 1))` }}>{formatDDayDate(d.full)}</th>;
                                    })}
                                  </tr>
                                </thead>
                                <tbody>
                                  <tr className="text-center">
                                    <td colSpan={2} className="text-center font-black bg-slate-50 text-slate-600 align-middle py-0.5 md:py-1 sticky left-0 z-[25] border-r border-b border-slate-300 shadow-[1px_0_0_#cbd5e1]" style={{ fontSize: `calc(${Math.max(9, fontSize - 1)}px * var(--mobile-scale, 1))` }}>비고</td>
                                    {chunk.map((d, i) => {
                                      ...
                                      return (
                                        <td key={cellId} 
                                          ...
                                          className={`p-0 align-middle text-center transition-colors border-r border-b border-slate-300 relative ${isSel ? 'ring-2 ring-indigo-500 ring-inset z-10 bg-indigo-50/80' : 'hover:bg-slate-50 bg-white'} ${isEditingThis ? 'cursor-text' : 'cursor-cell'}`}
                                        >
                                          <div className="w-full h-full flex flex-col justify-center items-center p-0.5 md:p-1 text-center min-h-[26px] md:min-h-[30px] relative">
                                            <textarea 
                                              ...
                                              style={{
                                                fontSize: `calc(${fontSize}px * var(--mobile-scale, 1))`, lineHeight: '1.2', opacity: isEditingThis ? 1 : 0,
                                                caretColor: (!isEditingThis && isActiveThis) ? 'transparent' : 'auto',
                                                cursor: (!isEditingThis && isActiveThis) ? 'default' : 'text',
                                                zIndex: isEditingThis ? 20 : 0
                                              }}
                                              className={`absolute inset-0 w-full h-full bg-white resize-none outline-none p-1 text-center font-bold text-slate-800 rounded align-middle auto-resize ${isActiveThis && !isEditingThis ? 'select-none' : ''}`} 
                                            />
                                            {!isEditingThis && (
                                              <div className="w-full h-full flex flex-col gap-1.5 px-0.5 py-0.5 justify-center min-h-[26px] md:min-h-[30px] relative z-10 pointer-events-none">
                                                <div style={{ fontSize: `calc(${fontSize}px * var(--mobile-scale, 1))`, lineHeight: '1.2' }} className="w-full h-full flex items-center justify-center whitespace-pre-wrap break-all font-bold text-slate-800">{val}</div>
                                              </div>
                                            )}
                                          </div>
                                        </td>
                                  ...
                                  {termScheduler.subjects.map((sub, sIdx) => {
                                    ...
                                    return (
                                      <tr key={sub} className="text-center align-middle">
                                        <td className="p-0.5 md:px-1 md:py-1 font-black text-center relative group bg-slate-50 align-middle break-all md:break-keep sticky left-0 z-[20] border-r border-b border-slate-300">
                                          <span style={{ fontSize: `calc(${Math.max(9, fontSize - 1)}px * var(--mobile-scale, 1))` }}>{sub}</span>
                                          <button onClick={() => removeSubjectRow(sub)} className="absolute right-0 top-0 md:right-0.5 md:top-0.5 opacity-0 group-hover:opacity-100 text-red-500 bg-white/80 hover:bg-white rounded p-0.5 transition-opacity text-center shadow-sm"><X size={10}/></button>
                                        </td>
                                        ...
                                        <td key={cellIdTb}
                                          ...
                                          className={`p-0 align-middle text-center transition-colors relative border-r border-b border-slate-300 sticky left-[45px] md:left-[60px] z-[20] shadow-[1px_0_0_#cbd5e1] ${isSelTb ? 'ring-2 ring-indigo-500 ring-inset bg-indigo-50/80' : 'hover:bg-slate-50 bg-white'} ${isEditingTb ? 'cursor-text' : 'cursor-cell'}`}
                                        >
                                          <div className="w-full h-full flex flex-col justify-center items-center p-0 text-center min-h-[30px] md:min-h-[50px] relative">
                                            <textarea 
                                                  ...
                                                  if (e.key === 'Enter') {
                                                    if (e.shiftKey || e.altKey) {
                                                      if (e.altKey && !e.shiftKey) {
                                                        ...
                                                        setTimeout(() => {
                                                          const targetEl = document.getElementById(`monthly-textarea-${rIdx}-${tbCIdx}`);
                                                          if (targetEl) {
                                                            targetEl.setSelectionRange(start + 1, start + 1);
                                                            autoResize({ target: targetEl });
                                                          }
                                              ...
                                              style={{
                                                fontSize: `calc(${fontSize}px * var(--mobile-scale, 1))`, lineHeight: '1.2', opacity: isEditingTb ? 1 : 0,
                                            ...
                                            />
                                            {!isEditingTb && (
                                              <div className="w-full h-full flex flex-col justify-center px-0.5 md:px-1 min-h-[30px] md:min-h-[40px] relative z-10 pointer-events-none bg-white">
                                                {tbVal.trim() === '' ? ( <span className="text-transparent select-none w-full h-full block pointer-events-none" style={{ fontSize: `calc(${fontSize}px * var(--mobile-scale, 1))` }}>.</span> ) : (
                                                  <div style={{ fontSize: `calc(${fontSize}px * var(--mobile-scale, 1))`, lineHeight: '1.2' }} className="font-bold text-slate-700 text-center w-full break-all md:break-words whitespace-pre-wrap pointer-events-none">{tbVal}</div>
                                                )}
                                              </div>
                                            )}
                                          </div>
                                        ...
                                        {chunk.map((d, i) => {
                                          ...
                                          return (
                                            <td key={cellId}
                                              ...
                                              className={`p-0 align-middle transition-colors relative border-r border-b border-slate-300 text-center ${isSel ? 'ring-2 ring-indigo-500 ring-inset z-10 bg-indigo-50/80' : 'hover:bg-slate-50 bg-white'} ${isEditingThis ? 'cursor-text' : 'cursor-cell'}`}
                                            >
                                              <div className="w-full h-full flex flex-col justify-center items-center p-0 text-center min-h-[30px] md:min-h-[50px] relative">
                                                <textarea 
                                                  id={`monthly-textarea-${rIdx}-${cIdx}`}
                                                  ...
                                                  className={`absolute inset-0 w-full h-full bg-white resize-none outline-none p-0.5 md:p-1 text-center font-bold text-slate-800 rounded align-middle auto-resize break-all md:break-words whitespace-pre-wrap ${isActiveThis && !isEditingThis ? 'select-none' : ''}`} 
                                                />
                                                {!isEditingThis && (
                                                  <div className="w-full h-full flex flex-col gap-1 px-0.5 md:px-1 py-0.5 md:py-1 justify-center min-h-[30px] md:min-h-[40px] relative z-10 pointer-events-auto cursor-default">
                                                    {val.trim() === '' ? ( <span className="text-transparent select-none w-full h-full block cursor-default pointer-events-none" style={{ fontSize: `calc(${fontSize}px * var(--mobile-scale, 1))` }}>.</span> ) : (
                                                      lines.map((line, idx) => (
                                                        <div key={idx} className="flex items-center justify-between border-b border-slate-200/50 last:border-0 px-0.5 md:px-1 py-0.5 w-full cursor-default pointer-events-auto">
                                                          <span style={{ fontSize: `calc(${fontSize}px * var(--mobile-scale, 1))`, lineHeight: '1.2' }} className="font-bold text-slate-800 text-left flex-1 break-all md:break-words whitespace-pre-wrap pointer-events-none">{line}</span>
                                                          <input type="checkbox" checked={termScheduler.checks[`${sub}-${d.full}-${idx}`] || false} 
                                                            ...
                                                            className="w-[10px] h-[10px] md:w-3 md:h-3 ml-1 mt-[1px] cursor-pointer accent-indigo-600 flex-shrink-0 relative z-30" 
                                                          />
                                                        </div>
                                                      ))
                                                    )}
                      ...
                      {termScheduler.subjects.length > 0 && (
                        <div className="text-left flex justify-center w-full text-center mt-2 md:mt-4 pb-2">
                          <div className="w-full max-w-4xl mx-auto">
                            <table className="w-full border-collapse text-[10px] md:text-[11px] bg-white border border-slate-300 shadow-sm text-center table-fixed align-middle">
                              <thead>
                                <tr className="bg-slate-50 font-black text-slate-700 text-center border-b border-slate-300" style={{ fontSize: `calc(${Math.max(9, fontSize - 1)}px * var(--mobile-scale, 1))` }}>
                                  <th className="w-[15%] md:w-[10%] py-2 md:py-3 align-middle text-center break-keep border-r border-slate-300">과목</th>
                                  <th className="w-[20%] md:w-[10%] align-middle text-center break-keep border-r border-slate-300">교재</th>
                                  <th className="w-[15%] md:w-[10%] align-middle text-center break-keep border-r border-slate-300">시작</th>
                                  <th className="w-[15%] md:w-[10%] align-middle text-center break-keep border-r border-slate-300">목표</th>
                                  <th className="w-[35%] md:w-[60%] align-middle text-center break-keep">달성도</th>
                                </tr>
                              </thead>
                              <tbody>
                                {termScheduler.subjects.map((sub) => {
                                  ...
                                  return rowData.map((data, index) => (
                                    <tr key={`status-${sub}-${index}`} className="hover:bg-slate-50 transition-colors text-center border-b border-slate-200 last:border-b-0" style={{ fontSize: `calc(${Math.max(8, fontSize - 2)}px * var(--mobile-scale, 1))` }}>
                                      {index === 0 && <td rowSpan={rowData.length} className="text-center font-black py-2 md:py-3 bg-slate-50/50 align-middle border-r border-slate-300"><span style={{ fontSize: `calc(${Math.max(9, fontSize - 1)}px * var(--mobile-scale, 1))` }}>{sub}</span></td>}
                                      <td className="p-1 md:p-2 text-center font-bold text-slate-700 align-middle break-all md:break-keep whitespace-pre-wrap border-r border-slate-200"><span style={{ fontSize: `calc(${Math.max(8, fontSize - 2)}px * var(--mobile-scale, 1))` }}>{data.tbName}</span></td>
                                      <td className="bg-slate-50/5 text-center font-black p-1 md:px-3 md:py-2 text-indigo-700 align-middle break-all md:break-keep whitespace-pre-wrap border-r border-slate-200"><span style={{ fontSize: `calc(${Math.max(8, fontSize - 2)}px * var(--mobile-scale, 1))` }}>{data.firstData}</span></td>
                                      <td className="bg-slate-50/5 text-center font-black p-1 md:px-3 md:py-2 text-rose-700 align-middle break-all md:break-keep whitespace-pre-wrap border-r border-slate-200"><span style={{ fontSize: `calc(${Math.max(8, fontSize - 2)}px * var(--mobile-scale, 1))` }}>{data.lastData}</span></td>
                                      <td className="p-1.5 md:p-3 text-center align-middle">
                                        <div className="relative w-[90%] md:w-full h-3 md:h-5 bg-slate-100 rounded-sm overflow-hidden border border-slate-200 mx-auto">
                                          <div className="absolute inset-y-0 left-0 bg-gradient-to-r from-emerald-400 to-emerald-300 transition-all duration-700 ease-out" style={{ width: `${data.percent}%` }} />
                                          <span className="absolute inset-y-0 left-0 right-0 flex items-center justify-center text-[7px] md:text-[9px] font-black text-slate-800 drop-shadow-sm">{data.percent}%</span>
                                        </div>
                                      </td>
                                    </tr>
                                  ));
                ...
                {activeTab === 'YEARLY' && (
                  // 💡 [연간 시트 인쇄 스타일/엑셀 그리드 화] 부피 큰 카드 제거, 3열 배치
                  <div className="animate-fade-in grid grid-cols-3 lg:grid-cols-6 gap-[1px] bg-slate-300 border border-slate-300 w-full max-w-6xl mx-auto shadow-sm mt-2 md:mt-4">
                    {yearlyPlan.map((plan, idx) => (
                      <div key={idx} className="bg-white flex flex-col h-full">
                        <div className="font-black text-slate-700 py-1.5 md:py-2 bg-slate-50 border-b border-slate-200 text-[10px] md:text-xs">
                          {idx + 1}월
                        </div>
                        <textarea 
                          value={plan || ''} 
                          ...
                          placeholder={`${idx + 1}월 계획`} 
                          style={{ fontSize: `calc(${Math.max(10, fontSize)}px * var(--mobile-scale, 1))`, lineHeight: '1.4' }}
                          className="w-full flex-1 p-1.5 md:p-2 outline-none focus:bg-indigo-50/10 transition-all font-bold resize-none text-center bg-transparent auto-resize min-h-[70px] md:min-h-[100px] break-words whitespace-pre-wrap placeholder:text-slate-300" 
                        />
                      </div>
                    ))}
                  </div>
              ...
              <div className="fixed bottom-6 right-6 md:bottom-10 md:right-10 z-50 flex flex-col items-end text-center print:hidden">
                {showAiModal ? (
                  <div className="w-[300px] sm:w-[360px] md:w-[420px] rounded-3xl shadow-2xl overflow-hidden border border-slate-200 bg-white animate-fade-in text-center">
                    <div className="bg-indigo-600 p-4 md:p-5 text-white flex justify-between items-center text-center">
                      <h3 className="font-extrabold text-base md:text-lg flex items-center justify-center gap-2 w-full text-center"><Sparkles size={20}/> AI 매직 플래너</h3>
                      <button onClick={() => setShowAiModal(false)}><X className="w-5 h-5 text-center" /></button>
                    </div>
                    <div className="p-4 md:p-6 text-center">
                      {aiFeedback && <div className="mb-4 md:mb-6 p-3 md:p-4 rounded-2xl text-center font-bold animate-pulse bg-emerald-50 text-emerald-600 border border-emerald-100 text-xs leading-relaxed text-center">{aiFeedback}</div>}
                      <form onSubmit={handleAiSubmit} className="relative mt-2 text-center text-center text-center">
                        <input type="text" value={aiPrompt} onChange={(e) => setAiPrompt(e.target.value)} placeholder="학습 명령을 입력하세요..." className="w-full pl-4 md:pl-5 pr-12 md:pr-14 py-3 md:py-4 rounded-2xl border-2 border-slate-200 focus:outline-none focus:border-indigo-500 transition-all font-bold text-sm md:text-base text-slate-800 text-center" disabled={isAiProcessing} />
                        <button type="submit" disabled={isAiProcessing || !aiPrompt.trim()} className="absolute right-2 top-1.5 md:top-2 p-2.5 md:p-3.5 bg-indigo-600 text-white rounded-xl hover:bg-indigo-700 transition-colors text-center"><Send size={16} className="md:w-5 md:h-5"/></button>
                      </form>
                    </div>
                  </div>
                ) : (
          ...
          {studentToDelete && (
            <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-fade-in text-center print:hidden" onClick={() => setStudentToDelete(null)}>
              <div className="w-full max-w-sm rounded-3xl shadow-2xl p-8 text-center bg-white text-center text-center text-center text-center text-center" onClick={(e) => e.stopPropagation()}>
                <div className="w-16 h-16 rounded-full bg-red-100 text-red-600 flex items-center justify-center mx-auto mb-4 text-center text-center text-center text-center text-center text-center text-center text-center text-center text-center"><Trash2 size={32} /></div>
      ...
      <style dangerouslySetInnerHTML={{ __html: `
        /* 💡 모바일 환경에서 폰트 사이즈 전체 15% 일괄 자동 축소 */
        :root { --mobile-scale: 1; }
        @media (max-width: 768px) {
          :root { --mobile-scale: 0.85; }
        }
