...
// 💡 [A/B/C 콤] 스마트 매크로 자동 치환 함수 (텍스트 시간표 원본 복구)
...
export default function App() {
  ...
  // 💡 [모바일 최적화 방어] 모바일 디바이스에서 세로 공간을 100%로 강제 유지
  const autoResize = (e) => { 
    if (e && e.target) { 
      if (typeof window !== 'undefined' && window.innerWidth <= 767 && (activeTab === 'WEEKLY' || activeTab === 'YEARLY')) {
        e.target.style.height = '100%';
        return;
      }
      e.target.style.height = 'auto'; 
      if (e.target.scrollHeight > 0) e.target.style.height = e.target.scrollHeight + 'px'; 
    }
  };
  ...
  useEffect(() => {
    const initAuth = async () => {
      try {
        ...
        if (error.code === 'auth/unauthorized-domain') setDbError("Vercel 도메인이 Firebase 인증 허용 목록에 없습니다.");
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(doc(db, 'planners', currentDocId), (docSnap) => {
      ...
      setDbError(`[읽기 차단됨] 권한이 만료되었거나 접근할 수 없습니다. (${error.message})`);
      setLoading(false);
    });
    return () => unsubscribe();
  ...
  useEffect(() => {
    ...
    const saveData = async () => {
      ...
      try {
        ...
        setDbError(`[쓰기 차단됨] 데이터 저장이 제한되었습니다.`);
      }
    };
...
[필수 규칙]
1. 사용자가 단순히 과목만 추가하라는 요청을 하면 new_subjects 배열에 담아 응답하세요.
2. 단순히 교재명만 나열하면 target을 "textbook"으로 설정하세요.
3. 특정 기간/요일에 분량을 나누어 달라고 요청하면 target을 "cell"로 설정하세요. 기간에 속하는 날짜마다 분배하여 각각 객체를 만드세요.
4. 과목명: 기존 등록된 과목 [${termScheduler.subjects.join(', ')}] 중 알맞은 과목을 매칭. 없으면 new_subjects에 포함.
5. target이 "cell"인 경우 date는 제공된 캘린더 날짜 배열을 참고하여 가장 일치하는 날짜를 YYYY-MM-DD 형식으로 입력하세요.
6. 복합 요청을 받으면 new_subjects 배열과 updates 배열에 각각 빠짐없이 반영하세요.`, 
...
plans 배열은 무조건 12개의 문자열로 구성. 요청되지 않은 달은 빈 문자열("")로 두세요.` 
  ...
  // 💡 화면 최적화를 위한 탭 플래그 계산 (주간, 연간은 화면 높이에 꽉 차야 함)
  const isHFullScreen = view === 'PLANNER' && (activeTab === 'WEEKLY' || activeTab === 'YEARLY');

  return (
    <>
      <div className={`print:hidden bg-slate-50 text-slate-800 transition-colors duration-300 ${isHFullScreen ? 'h-screen h-[100dvh] flex flex-col overflow-hidden' : 'min-h-screen flex flex-col'}`}>
        <div className={`w-full mx-auto ${isHFullScreen ? 'flex-1 flex flex-col min-h-0' : 'flex-1 flex flex-col'}`}>
          ...
          {view === 'LANDING' && (
            <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4 text-center flex-1">
          ...
          {view === 'PLANNER' && (
            <div className="flex flex-col h-full w-full relative flex-1">
              ...
              <main className={`flex-1 min-h-0 w-full mx-auto relative text-center flex flex-col ${isHFullScreen ? 'p-1 md:p-2 overflow-hidden md:overflow-y-auto' : 'p-2 md:p-6 pb-24 overflow-y-auto'} custom-scrollbar`}>
                
                {activeTab === 'WEEKLY' && (
                  <div className="animate-fade-in flex flex-col text-center h-full">
                    <div className="flex-1 flex flex-col min-h-0">
                      <div className="p-1 md:p-2 rounded-xl shadow-sm border border-slate-200 bg-white flex flex-col h-full relative z-30 min-h-0">
                        ...
                        <div className="w-full flex-1 relative select-none rounded-lg border-y md:border-2 border-slate-200 bg-white shadow-inner text-center overflow-x-hidden custom-scrollbar z-10 weekly-scroll-area" onMouseLeave={handleMouseUp}>
                          <table className="w-full h-full min-h-full text-center border-collapse table-fixed weekly-table">
                            <thead className="z-20 shadow-sm border-b-2 border-slate-200 text-slate-800 bg-slate-50 sticky top-0">
                              <tr style={{ height: '30px' }}>
                                <th className={`border-r border-slate-200 uppercase font-black z-20 align-middle transition-colors duration-200 w-10 md:w-14 weekly-header-text ${wBounds ? 'bg-indigo-100 text-indigo-700' : 'text-slate-400'}`} style={{ fontSize: `${Math.max(8, fontSize - 2)}px` }}>
                                ...
                                {DAYS.map((d, i) => {
                                  ...
                                  return (
                                    <th key={d} className={`font-black border-r border-slate-200 z-20 align-middle transition-colors duration-200 py-0 px-0 weekly-header-text ${textColor} ${bgColor}`} style={{ fontSize: `${Math.max(9, fontSize - 1)}px` }}>
                              ...
                              {timetable.map((row) => {
                                ...
                                return (
                                  <tr key={row.id} className="group text-center">
                                    <td className={`p-0 border-b border-r border-slate-200 align-middle transition-colors duration-200 select-none weekly-td ${timeBgClass}`}>
                                      <div className={`flex flex-col items-center justify-center w-full h-full min-h-[20px] md:min-h-[28px] weekly-time-text ${timeTextClass}`} style={{ fontSize: `${Math.max(8, fontSize - 2)}px` }}>
                                        <span>{row.time}</span>
                                      </div>
                                    </td>
                                    {DAYS.map((day) => {
                                      ...
                                      return (
                                        <td 
                                          key={day} 
                                          className={`p-0 relative align-top border-b border-r border-slate-200 transition-colors duration-200 weekly-td ${isSelected ? 'ring-2 ring-indigo-500 ring-inset z-10' : ''} hover:bg-indigo-50/30 ${isEditingThis ? 'cursor-text' : 'cursor-cell'}`} 
                                        ...
                                        >
                                          <div className="w-full h-full flex flex-col items-center justify-center p-0 md:p-1 text-center min-h-[20px] md:min-h-[28px] weekly-td-inner">
                                            <textarea 
                                              ...
                                              onKeyDown={(e) => {
                                                ...
                                                if (!isEditingThis && isActiveThis) {
                                                  ...
                                                  // 💡 [Shift+Enter 및 Alt+Enter 통합 줄바꿈 처리]
                                                  if (e.key === 'Enter') {
                                              ...
                                              className={`w-full p-0 md:p-1 m-0 text-center bg-transparent resize-none outline-none overflow-hidden font-bold align-middle break-words whitespace-pre-wrap auto-resize weekly-textarea ${(isActiveThis && !isEditingThis) ? 'select-none' : ''}`} 
                                              rows={1}
                                            />
                                          </div>
                                        </td>
                ...
                {activeTab === 'MONTHLY' && (
                  <div className="animate-fade-in flex flex-col gap-4 md:gap-6 text-center w-full overflow-x-hidden">
                    <div className="p-1 md:p-6 rounded-3xl border border-slate-200 bg-white shadow-sm w-full text-center">
                      <div className="flex items-center justify-between mb-4 md:mb-6 px-1 md:px-2 text-center mt-2 md:mt-0">
                        <div className="flex items-center gap-2 md:gap-4 text-center">
                          <div className="flex gap-1 md:gap-2 text-center">
                            <button onClick={handlePrev4Weeks} className="p-1 md:p-2 rounded-xl bg-slate-100 hover:bg-slate-200 transition-colors text-center flex items-center justify-center"><ChevronLeft size={16} className="md:w-5 md:h-5"/></button>
                            <button onClick={handleNext4Weeks} className="p-1 md:p-2 rounded-xl bg-slate-100 hover:bg-slate-200 transition-colors text-center flex items-center justify-center"><ChevronRight size={16} className="md:w-5 md:h-5"/></button>
                          </div>
                        ...
                        <div className="flex gap-1 md:gap-2 text-center">
                          <button onClick={() => { const name = prompt("추가할 과목명을 입력하세요\n(예: 국어, 수학, 영어)"); if(name) name.split(',').forEach(n => { if (n.trim()) addSubjectRow(n.trim()); }); }} className="flex items-center gap-1 md:gap-1.5 px-2 md:px-5 py-1.5 md:py-2.5 bg-indigo-600 text-white rounded-lg md:rounded-xl font-extrabold text-[10px] md:text-sm hover:bg-indigo-700 shadow-md transition-all text-center"><Plus size={12} className="md:w-4 md:h-4"/> <span className="hidden md:inline">과목 추가</span></button>
                          <button onClick={() => setShowResetConfirm(true)} className="flex items-center gap-1 md:gap-1.5 px-2 md:px-5 py-1.5 md:py-2.5 bg-red-50 text-red-600 border border-red-100 rounded-lg md:rounded-xl font-extrabold text-[10px] md:text-sm hover:bg-red-100 transition-all text-center"><Trash2 size={12} className="md:w-4 md:h-4"/> <span className="hidden md:inline">일정 초기화</span></button>
                        </div>
                      </div>
                      ...
                      {[0, 1].map((blockIdx) => {
                        ...
                        return (
                          <div key={blockIdx} className="w-full relative select-none overflow-x-auto pb-3 monthly-scroll-container custom-scrollbar" onMouseLeave={handleMouseUp}>
                            <div className="min-w-[800px] md:min-w-full monthly-table-wrapper">
                              <table className="w-full border-collapse mb-4 md:mb-10 text-[9px] md:text-[11px] table-fixed text-center align-middle monthly-table">
                                <thead>
                                  ...
                                  <tr className="bg-white text-center">
                                    ...
                                    {chunk.map((d, i) => {
                                      ...
                                      return (
                                        ...
                                        >
                                          <div className="w-full h-full flex flex-col justify-center items-center p-1 text-center min-h-[22px] md:min-h-[30px] relative">
                                            <textarea 
                                              ...
                                              onKeyDown={(e) => { 
                                                ...
                                                if (!isEditingThis && isActiveThis) {
                                                  ...
                                                  if (e.key === 'Enter') {
                                                    // 💡 [Shift+Enter 및 Alt+Enter 통합 줄바꿈]
                                                    if (e.shiftKey || e.altKey) {
                                                      e.preventDefault();
                                                      const target = e.currentTarget;
                                                      const start = target.selectionStart;
                                            ...
                                            />
                                            {!isEditingThis && (
                                              <div className="w-full h-full flex flex-col gap-1.5 px-1 py-1 justify-center min-h-[22px] md:min-h-[30px] relative z-10 pointer-events-none">
                                                <div style={{ fontSize: `${fontSize}px`, lineHeight: '1.3' }} className="w-full h-full flex items-center justify-center whitespace-pre-wrap font-bold text-slate-800 monthly-text-content break-all md:break-words">{val}</div>
                                              </div>
                                            )}
                                          </div>
                                        </td>
                                      ...
                                      <tr key={sub} className="text-center align-middle">
                                        ...
                                        >
                                          <div className="w-full h-full flex flex-col justify-center items-center p-0 text-center min-h-[30px] md:min-h-[50px] relative">
                                            ...
                                            />
                                            {!isEditingTb && (
                                              <div className="w-full h-full flex flex-col justify-center px-0.5 md:px-1 min-h-[30px] md:min-h-[40px] relative z-10 pointer-events-none">
                                                {tbVal.trim() === '' ? ( <span className="text-transparent select-none w-full h-full block pointer-events-none" style={{ fontSize: `${fontSize}px` }}>.</span> ) : (
                                                  <div style={{ fontSize: `${fontSize}px`, lineHeight: '1.3' }} className="font-bold text-slate-700 text-center w-full break-all md:break-words whitespace-pre-wrap pointer-events-none monthly-text-content">{tbVal}</div>
                                                )}
                                              </div>
                                            )}
                                          </div>
                                          ...
                                          return (
                                            ...
                                            >
                                              <div className="w-full h-full flex flex-col justify-center items-center p-0 text-center min-h-[30px] md:min-h-[50px] relative">
                                                ...
                                                />
                                                {!isEditingThis && (
                                                  <div className="w-full h-full flex flex-col gap-0.5 md:gap-1 px-0.5 md:px-1 py-0.5 md:py-1 justify-center min-h-[30px] md:min-h-[40px] relative z-10 pointer-events-auto cursor-default">
                                                    {val.trim() === '' ? ( <span className="text-transparent select-none w-full h-full block cursor-default pointer-events-none" style={{ fontSize: `${fontSize}px` }}>.</span> ) : (
                                                      lines.map((line, idx) => (
                                                        <div key={idx} className="flex items-center justify-center gap-0.5 md:gap-1 bg-white/70 rounded p-0 md:px-1 md:py-0.5 shadow-sm border border-black/5 mx-auto w-[98%] cursor-default pointer-events-auto">
                                                          <span style={{ fontSize: `${fontSize}px`, lineHeight: '1.2' }} className="font-black text-slate-800 text-center flex-1 break-all md:break-words whitespace-pre-wrap pointer-events-none monthly-text-content">{line}</span>
                      ...
                      {termScheduler.subjects.length > 0 && (
                        <div className="text-left flex justify-center w-full text-center mt-2 md:mt-6 overflow-x-auto custom-scrollbar pb-2 monthly-status-container">
                          <div className="min-w-[500px] md:min-w-full w-full monthly-status-wrapper">
                            <table className="w-full border-collapse text-[10px] md:text-[11px] shadow-md rounded-xl md:rounded-2xl overflow-hidden border border-slate-200 text-center table-fixed align-middle monthly-status-table">
                              <thead>
                ...
                {activeTab === 'YEARLY' && (
                  <div className="animate-fade-in grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-2 md:gap-4 text-center yearly-grid">
                    {yearlyPlan.map((plan, idx) => (
                      <div key={idx} className="p-2 md:p-6 rounded-2xl md:rounded-3xl border border-slate-200 bg-white shadow-sm transition-all hover:shadow-md text-center yearly-mobile-item">
                        <h4 className="font-black text-indigo-600 mb-1 md:mb-3 text-[10px] md:text-base text-center flex-none">{idx + 1}월 계획</h4>
                        <div className="yearly-textarea-wrapper flex-1 w-full relative">
                          <textarea 
                            id={`yearly-textarea-${idx}`}
                            value={plan || ''} 
                            ...
                            className="w-full p-1.5 md:p-4 rounded-lg md:rounded-xl border border-slate-100 outline-none focus:border-indigo-500 transition-all font-bold resize-none text-center bg-transparent auto-resize break-words whitespace-pre-wrap yearly-textarea" 
                          />
                        </div>
                      </div>
                    ))}
                ...
                {showAiModal ? (
                  <div className="w-[300px] sm:w-[360px] md:w-[420px] rounded-3xl shadow-2xl overflow-hidden border border-slate-200 bg-white animate-fade-in text-center">
                    ...
                    <div className="p-4 md:p-6 text-center">
                      {aiFeedback && <div className="mb-4 md:mb-6 p-4 rounded-2xl font-bold animate-pulse bg-emerald-50 text-emerald-600 border border-emerald-100 text-[10px] md:text-xs leading-relaxed">{aiFeedback}</div>}
                      <form onSubmit={handleAiSubmit} className="relative mt-2 text-center">
                        <input type="text" value={aiPrompt} onChange={(e) => setAiPrompt(e.target.value)} placeholder="학습 명령을 입력하세요..." className="w-full pl-4 pr-12 py-3 md:py-4 rounded-2xl border-2 border-slate-200 focus:outline-none focus:border-indigo-500 transition-all font-bold text-slate-800 text-[10px] md:text-base text-center" disabled={isAiProcessing} />
                        <button type="submit" disabled={isAiProcessing || !aiPrompt.trim()} className="absolute right-1.5 top-1.5 md:right-2 md:top-2 p-2 md:p-3.5 bg-indigo-600 text-white rounded-xl hover:bg-indigo-700 transition-colors"><Send size={18} className="md:w-5 md:h-5"/></button>
                      </form>
                    </div>
                  </div>
                ) : (
                  <button onClick={() => setShowAiModal(true)} className="flex items-center justify-center w-12 h-12 md:w-16 md:h-16 bg-indigo-600 text-white rounded-full shadow-2xl hover:scale-110 active:scale-95 transition-all text-center"><Sparkles className="w-6 h-6 md:w-7 md:h-7 text-center" /></button>
                )}
              </div>
            </div>
          )}
          ...
          {showResetConfirm && (
            <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-fade-in text-center print:hidden" onClick={() => setShowResetConfirm(false)}>
              <div className="w-full max-w-xs rounded-3xl shadow-2xl p-8 text-center bg-white text-center text-center text-center text-center text-center text-center">
                <div className="w-16 h-16 rounded-full bg-red-100 text-red-600 flex items-center justify-center mx-auto mb-4 text-center text-center text-center text-center text-center text-center text-center"><AlertCircle size={32} /></div>
                <h3 className="font-black text-xl mb-2 text-center text-center text-center text-center text-center text-center text-center">데이터 초기화</h3>
                <p className="text-sm mb-8 text-slate-500 font-bold text-center text-center text-center text-center text-center text-center text-center">현재 탭의 데이터를 모두 지울까요?</p>
                <div className="flex gap-3 text-center text-center text-center text-center text-center text-center text-center">
                  <button onClick={() => setShowResetConfirm(false)} className="flex-1 py-3 bg-slate-100 rounded-xl font-bold text-slate-600 text-center text-center text-center text-center text-center text-center text-center text-center">취소</button>
                  <button onClick={executeResetTimetable} className="flex-1 py-3 bg-red-600 text-white rounded-xl font-black shadow-lg text-center text-center text-center text-center text-center text-center text-center text-center">확인</button>
                </div>
              </div>
            </div>
          )}
          ...
          {showLogoutConfirm && (
            <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-fade-in text-center print:hidden" onClick={() => setShowLogoutConfirm(false)}>
              <div className="w-full max-w-sm rounded-3xl shadow-2xl p-8 text-center bg-white text-center text-center" onClick={(e) => e.stopPropagation()}>
                <div className="w-16 h-16 rounded-full bg-slate-100 text-slate-600 flex items-center justify-center mx-auto mb-4 text-center text-center text-center text-center text-center text-center text-center text-center"><LogOut size={32} /></div>
                <h3 className="font-black text-xl mb-2 text-center text-center text-center text-center text-center text-center text-center text-center text-center">로그아웃</h3>
                <p className="text-sm mb-8 text-slate-500 font-bold text-center text-center text-center text-center text-center text-center text-center text-center text-center text-center">정말 로그아웃 하시겠습니까?</p>
                <div className="flex gap-3 text-center text-center text-center text-center text-center text-center text-center text-center text-center">
                  <button onClick={() => setShowLogoutConfirm(false)} className="flex-1 py-3 bg-slate-100 rounded-xl font-bold text-slate-600 text-center text-center text-center text-center text-center text-center text-center text-center text-center text-center">취소</button>
                  <button onClick={executeLogout} className="flex-1 py-3 bg-indigo-600 text-white rounded-xl font-black shadow-lg shadow-indigo-100 text-center text-center text-center text-center text-center text-center text-center text-center text-center text-center">로그아웃</button>
                </div>
              </div>
            </div>
          )}
          ...
          {studentToDelete && (
            <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-fade-in text-center print:hidden" onClick={() => setStudentToDelete(null)}>
              <div className="w-full max-w-sm rounded-3xl shadow-2xl p-8 text-center bg-white text-center text-center text-center text-center text-center" onClick={(e) => e.stopPropagation()}>
                ...
                <h3 className="font-black text-xl mb-2 text-center text-center text-center text-center text-center text-center text-center text-center text-center text-center text-center">데이터 삭제</h3>
                <p className="text-sm mb-8 text-slate-500 font-bold text-center text-center text-center text-center text-center text-center text-center text-center text-center text-center text-center">이 시트를 삭제하시겠습니까?</p>
                <div className="flex gap-3 text-center text-center text-center text-center text-center text-center text-center text-center text-center text-center text-center text-center">
                  ...
                  <button onClick={executeDeleteStudent} className="flex-1 py-3 bg-red-600 text-white rounded-xl font-black shadow-lg text-center text-center text-center text-center text-center text-center text-center text-center text-center text-center text-center text-center">삭제</button>
                </div>
              </div>
            </div>
          )}
      ...
      <style dangerouslySetInnerHTML={{ __html: `
        ...
        /* 💡 모바일 환경 맞춤형 가독성 압도적 최적화 CSS (PC 환경 완벽 격리) */
        @media (max-width: 767px) {
          /* 1. WEEKLY 모바일 최적화 (높이 100% 강제, 세로 팽창 방지, 폰트 clamp) */
          .weekly-scroll-area { overflow: hidden !important; }
          .weekly-table { height: 100% !important; table-layout: fixed !important; }
          .weekly-table tr { height: 3.125% !important; }
          .weekly-td { position: relative !important; padding: 0 !important; height: 100% !important; }
          .weekly-td-inner { position: absolute !important; inset: 0 !important; height: 100% !important; min-height: 0 !important; padding: 1px !important; }
          .weekly-textarea { position: absolute !important; inset: 0 !important; height: 100% !important; max-height: 100% !important; overflow: hidden !important; font-size: clamp(6px, 1.8vw, 10px) !important; line-height: 1.1 !important; padding: 1px !important; resize: none !important; }
          .weekly-time-text { font-size: clamp(6px, 1.8vw, 9px) !important; }
          .weekly-header-text { font-size: clamp(7px, 2vw, 11px) !important; }

          /* 2. MONTHLY 모바일 최적화 (가로폭 100%로 가로 스크롤 제거, 세로 스크롤 허용) */
          .monthly-scroll-container { overflow-x: hidden !important; overflow-y: auto !important; width: 100% !important; }
          .monthly-table-wrapper { min-width: 100% !important; width: 100% !important; }
          .monthly-table { table-layout: fixed !important; width: 100% !important; min-width: 100% !important; }
          .monthly-mobile-text { font-size: clamp(5px, 1.8vw, 8px) !important; padding: 1px !important; word-break: break-all !important; line-height: 1.1 !important; }
          .monthly-text-content { font-size: clamp(5px, 1.8vw, 8px) !important; line-height: 1.1 !important; }
          .monthly-table textarea { font-size: clamp(5px, 1.8vw, 8px) !important; line-height: 1.1 !important; padding: 1px !important; min-height: 18px !important; }

          .monthly-status-container { overflow-x: hidden !important; width: 100% !important; }
          .monthly-status-wrapper { min-width: 100% !important; width: 100% !important; }
          .monthly-status-table { min-width: 100% !important; width: 100% !important; table-layout: fixed !important; }
          .monthly-status-table th, .monthly-status-table td { font-size: clamp(6px, 2vw, 9px) !important; padding: 1px !important; }
          .monthly-status-table span { font-size: clamp(6px, 2vw, 9px) !important; }

          /* 3. YEARLY 모바일 최적화 (3열 4행 그리드로 한눈에 꽉 차게) */
          .yearly-grid { display: grid !important; grid-template-columns: repeat(3, 1fr) !important; grid-template-rows: repeat(4, 1fr) !important; height: 100% !important; gap: 4px !important; }
          .yearly-mobile-item { display: flex !important; flex-direction: column !important; padding: 4px !important; height: 100% !important; min-height: 0 !important; }
          .yearly-mobile-item h4 { font-size: 10px !important; margin-bottom: 2px !important; }
          .yearly-textarea-wrapper { flex: 1 !important; position: relative !important; width: 100% !important; height: 100% !important; min-height: 0 !important; }
          .yearly-textarea { position: absolute !important; inset: 0 !important; height: 100% !important; max-height: 100% !important; overflow: hidden !important; font-size: clamp(8px, 1.5vh, 12px) !important; line-height: 1.2 !important; padding: 2px !important; resize: none !important; }
        }
