...
export default function App() {
  ...
  const [showLogoutConfirm, setShowLogoutConfirm] = useState(false); // 🟢 [추가] 로그아웃 확인 모달 상태
  const [studentToDelete, setStudentToDelete] = useState(null); // 🟢 [추가] 삭제할 학생 ID 상태 저장
  ...
  // 🟢 [수정] 학생 플래너 영구 삭제 함수 (기본 팝업 제거 -> 커스텀 모달 띄우기)
  const handleDeleteStudent = (e, studentId) => {
    e.stopPropagation(); // 카드 클릭 시 플래너로 넘어가는 것을 방지합니다.
    setStudentToDelete(studentId); // 삭제할 학생의 ID를 저장하고 모달을 엽니다.
  };
  ...
  const executeDeleteStudent = async () => {
    ...
    try {
      ...
      setStudentToDelete(null); // 삭제 완료 후 모달 닫기
    } catch (error) {
  ...
  const renderCalendar = () => {
    ...
    return (
      <div className="grid grid-cols-7 gap-1 text-center text-sm md:text-base">
        {['일', '월', '화', '수', '목', '금', '토'].map((d) => (
          <div key={d} className={`font-bold py-2 ${d === '일' ? 'text-red-500' : d === '토' ? 'text-blue-500' : ''}`}>{d}</div>
        ))}
        {days.map((d, i) => (
          <div
            key={i}
            className={`h-12 md:h-16 border rounded-lg flex items-center justify-center transition-colors ${
              !d ? 'bg-transparent border-none' : darkMode ? 'border-slate-700 bg-slate-800' : 'border-slate-200 bg-white'
            } ${d === new Date().getDate() && month === new Date().getMonth() ? 'ring-2 ring-indigo-500 font-extrabold text-indigo-500' : ''}`}
          >
            {d}
          </div>
        ))}
    ...
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4">
        <div className="max-w-md w-full bg-white rounded-3xl shadow-xl p-8 border border-slate-100">
          ...
          <div className="mb-8"><h2 className="text-3xl font-extrabold text-slate-800 mb-2">{isStudent ? '학생 이름 입력' : '관리자 로그인'}</h2><p className="text-slate-500">{isStudent ? '본인의 이름을 입력하고 플래너를 시작하세요.' : '선생님 전용 비밀번호를 입력해주세요.'}</p></div>
          <form onSubmit={isStudent ? handleStudentLogin : handleTeacherLogin} className="space-y-6">
            <div className="space-y-2">
              <label className="text-sm font-bold text-slate-700 ml-1">{isStudent ? '이름' : '비밀번호'}</label>
              <input type={isStudent ? 'text' : 'password'} value={isStudent ? studentName : teacherPassword} onChange={(e) => isStudent ? setStudentName(e.target.value) : setTeacherPassword(e.target.value)} placeholder={isStudent ? '예: 홍길동' : '비밀번호를 입력하세요'} className="w-full p-4 border-2 border-slate-200 rounded-2xl outline-none focus:ring-4 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all text-lg font-medium" autoFocus />
            </div>
            {errorMsg && !isStudent && <div className="p-3 bg-red-50 text-red-600 rounded-xl text-sm font-bold flex items-center gap-2"><AlertCircle size={16}/> {errorMsg}</div>}
            <button type="submit" className={`w-full text-white p-5 rounded-2xl font-extrabold text-lg transition-all transform hover:-translate-y-1 shadow-lg ${isStudent ? 'bg-gradient-to-r from-indigo-600 to-indigo-500 hover:from-indigo-700 hover:to-indigo-600 shadow-indigo-200' : 'bg-slate-800 hover:bg-slate-900 shadow-slate-200'}`}>{isStudent ? '내 플래너 시작하기' : '대시보드 접속'}</button>
          </form>
        </div>
      </div>
    );
  ...
  return (
    <div className={`min-h-screen transition-colors duration-300 ${darkMode ? 'bg-slate-900 text-slate-100' : 'bg-slate-50 text-slate-800'}`}>
      ...
      {/* 🟢 [추가] 학생 삭제 확인 커스텀 모달 */}
      {studentToDelete && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-fade-in" onClick={() => setStudentToDelete(null)}>
          <div className={`w-full max-w-sm rounded-3xl shadow-2xl p-8 relative transform transition-all ${darkMode ? 'bg-slate-800 text-white border border-slate-700' : 'bg-white text-slate-800'}`} onClick={(e) => e.stopPropagation()}>
            <div className={`w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-4 ${darkMode ? 'bg-red-900/30 text-red-500' : 'bg-red-100 text-red-600'}`}>
              <Trash2 size={32} />
            </div>
