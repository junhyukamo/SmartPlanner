...
// 학생 정보 데이터 마이그레이션 호환 처리
const repairStudentInfo = (info) => {
  if (!info) return JSON.parse(JSON.stringify(defaultStudentInfo));
  const newInfo = { ...info };
  ['topHeaders', 'websites', 'electives', 'midHeaders', 'schoolGrades', 'mockGrades'].forEach(key => {
...
export default function App() {
  ...
  const moveFocusInfo = (section, rIdx, cIdx, dir) => {
    ...
    if (!is2D) {
      ...
      const elId = `info-${section}-0-${nextIdx}`;
      setTimeout(() => { const el = document.getElementById(elId); if (el) { el.focus(); el.setSelectionRange(el.value.length, el.value.length); } }, 0);
      return;
    }
    ...
    const elId = `info-${section}-${nextR}-${nextC}`;
    setTimeout(() => { const el = document.getElementById(elId); if (el) { el.focus(); el.setSelectionRange(el.value.length, el.value.length); } }, 0);
  };
  ...
  }, [activeTab, view, currentDocId, loading, timetable, fontSize, termScheduler, editingCell, flatRows, studentInfo]);
    ...
    const unsubscribe = onSnapshot(doc(db, 'planners', currentDocId), (docSnap) => {
      try {
        ...
        if (docSnap.exists()) {
          ...
          // 💡 서버에서 데이터 로드 시 안전하게 구조 병합
          setStudentInfo(data.studentInfo ? repairStudentInfo(data.studentInfo) : JSON.parse(JSON.stringify(defaultStudentInfo)));
  ...
  useEffect(() => {
    ...
    const saveData = async () => {
      ...
      try {
        await setDoc(doc(db, 'planners', currentDocId), { timetable, todos, dDay, yearlyPlan, termScheduler, studentInfo, colorRules, lastUpdated: new Date().toISOString(), ...(isActuallyName && { studentName }) }, { merge: true });
      } catch(e) {
  ...
  useEffect(() => {
    const handleCopy = (e) => {
      ...
      if (activeTab === 'YEARLY' || activeTab === 'INFO') return; // 학생 정보 탭 제외
  ...
  // 💡 [추가] 학생 정보 키보드 동작(칸 추가, 줄바꿈, 이동) 핸들러
  const handleInfoKeyDown = (e, section, rIdx, cIdx) => {
    if (e.nativeEvent.isComposing && e.key !== 'Escape') return;

    if (e.key === 'Enter') {
        // Shift+Enter 칸 추가 기능 (학교성적/모의고사 테이블에만 적용)
        if (e.shiftKey) {
            e.preventDefault();
            if (section !== 'schoolGrades' && section !== 'mockGrades') {
    ...
    // 빈 칸에서 Backspace 시 해당 행 통째로 삭제
    else if (e.key === 'Backspace' && e.currentTarget.value === '') {
        if ((section === 'schoolGrades' || section === 'mockGrades') && studentInfo[section].length > 1) {
            e.preventDefault();
            saveToHistory();
            const newInfo = { ...studentInfo };
            newInfo[section].splice(rIdx, 1);
            setStudentInfo(newInfo);
            if (rIdx > 0) moveFocusInfo(section, rIdx - 1, cIdx, 'DOWN'); 
        }
    } 
    // 자유로운 방향키 이동
          ...
          {view === 'PLANNER' && (
            <div className="flex flex-col h-full w-full relative">
              ...
              <main className={`flex-1 min-h-0 w-full mx-auto relative text-center flex flex-col ${activeTab === 'WEEKLY' ? 'p-1 md:p-2' : 'p-2 md:p-6 pb-24'} overflow-y-auto custom-scrollbar`}>
                                          ...
                                          <div className="w-full h-full flex flex-col justify-center items-center p-0 text-center min-h-[36px] md:min-h-[50px] relative">
                                            ...
                                            {!isEditingTb && (
                                              <div className="w-full h-full flex flex-col justify-center px-0.5 md:px-1 min-h-[36px] md:min-h-[40px] relative z-10 pointer-events-none">
                                                {tbName.trim() === '' ? ( <span className="text-transparent select-none w-full h-full block pointer-events-none" style={{ fontSize: `${fontSize}px` }}>.</span> ) : (
                                                  <div style={{ fontSize: `${fontSize}px`, lineHeight: '1.3' }} className="font-bold text-slate-700 text-center w-full break-all md:break-words whitespace-pre-wrap pointer-events-none">{tbName}</div>
                                                )}
                                              </div>
                                            )}
                                          </div>
                            ...
                            <table className="w-full border-collapse text-[10px] md:text-[11px] shadow-md rounded-xl md:rounded-2xl overflow-hidden border border-slate-200 text-center table-fixed align-middle">
                              ...
                              <tbody>
                                {flatRows.map((rowInfo) => {
                                  ...
                                  return (
                                    <tr key={`status-${sub}-${tbIdx}`} className="bg-white hover:bg-slate-50 transition-colors text-center" style={{ fontSize: `${Math.max(9, fontSize - 1)}px` }}>
                                      {isFirst && <td rowSpan={rowSpan} className="border border-slate-200 text-center font-black py-2 md:py-3 bg-slate-50/50 align-middle"><span style={{ fontSize: `${fontSize}px` }}>{sub}</span></td>}
                                      <td className="border border-slate-200 p-1 md:p-2 text-center font-bold text-slate-700 align-middle break-words whitespace-pre-wrap"><span style={{ fontSize: `${fontSize}px` }}>{tbName || '-'}</span></td>
                                      <td className="border border-slate-200 bg-slate-50/5 text-center font-black p-1 md:px-3 md:py-2 text-indigo-700 align-middle break-words whitespace-pre-wrap"><span style={{ fontSize: `${fontSize}px` }}>{firstData}</span></td>
                                      <td className="border border-slate-200 bg-slate-50/5 text-center font-black p-1 md:px-3 md:py-2 text-rose-700 align-middle break-words whitespace-pre-wrap"><span style={{ fontSize: `${fontSize}px` }}>{lastData}</span></td>
                ...
                {/* 💡 [학생 정보 탭 렌더링 구현] */}
                {activeTab === 'INFO' && (
                  <div className="animate-fade-in flex flex-col gap-4 w-full text-center overflow-x-hidden p-2 md:p-4 bg-white rounded-3xl border border-slate-200 shadow-sm">
                    {/* 상단: 웹사이트 및 선택과목 */}
                      ...
                      <table className="w-full border-collapse text-center table-fixed min-w-[700px]">
                        <thead>
                          <tr className="bg-slate-50 text-slate-700">
                            {[0, 1, 2].map(i => (
                              <th key={`topH-${i}`} className={`border border-slate-300 p-1 md:p-2 w-[10%] align-middle`}>
                                <textarea id={`info-topHeaders-0-${i}`} value={studentInfo.topHeaders[i]} onChange={(e) => handleInfoChange('topHeaders', i, null, e.target.value)} onKeyDown={(e) => handleInfoKeyDown(e, 'topHeaders', i, null)} onInput={autoResize} onFocus={handleFocus} onBlur={(e) => handleBlur(e, null, null, true, null, null)} className="w-full h-full bg-transparent resize-none outline-none text-center font-black auto-resize" style={{ fontSize: `${Math.max(10, fontSize)}px` }} rows={1} />
                              </th>
                            ))}
                            <th className={`border border-slate-300 p-1 md:p-2 w-[30%] align-middle`}>
                              <textarea id={`info-topHeaders-0-3`} value={studentInfo.topHeaders[3]} onChange={(e) => handleInfoChange('topHeaders', 3, null, e.target.value)} onKeyDown={(e) => handleInfoKeyDown(e, 'topHeaders', 3, null)} onInput={autoResize} onFocus={handleFocus} onBlur={(e) => handleBlur(e, null, null, true, null, null)} className="w-full h-full bg-transparent resize-none outline-none text-center font-black auto-resize" style={{ fontSize: `${Math.max(10, fontSize)}px` }} rows={1} />
                            </th>
                            <th className={`border border-slate-300 p-1 md:p-2 w-[40%] align-middle`}>
                              <textarea id={`info-topHeaders-0-4`} value={studentInfo.topHeaders[4]} onChange={(e) => handleInfoChange('topHeaders', 4, null, e.target.value)} onKeyDown={(e) => handleInfoKeyDown(e, 'topHeaders', 4, null)} onInput={autoResize} onFocus={handleFocus} onBlur={(e) => handleBlur(e, null, null, true, null, null)} className="w-full h-full bg-transparent resize-none outline-none text-center font-black auto-resize" style={{ fontSize: `${Math.max(10, fontSize)}px` }} rows={1} />
                            </th>
                          </tr>
                        </thead>
                        <tbody>
                          {studentInfo.websites.map((row, rIdx) => (
                            <tr key={rIdx} className="hover:bg-slate-50 transition-colors">
                              {[0, 1, 2].map(cIdx => (
                                <td key={`web-${rIdx}-${cIdx}`} className="border border-slate-300 p-1 align-middle text-center">
                                  <textarea id={`info-websites-${rIdx}-${cIdx}`} value={row[cIdx]} onChange={(e) => handleInfoChange('websites', rIdx, cIdx, e.target.value)} onKeyDown={(e) => handleInfoKeyDown(e, 'websites', rIdx, cIdx)} onInput={autoResize} onFocus={handleFocus} onBlur={(e) => handleBlur(e, null, null, true, null, null)} className="w-full h-full bg-transparent resize-none outline-none text-center font-bold text-slate-700 auto-resize" style={{ fontSize: `${fontSize}px` }} rows={1} />
                                </td>
                              ))}
                              {rIdx === 0 && (
                                <td rowSpan={studentInfo.websites.length} className="border border-slate-300 p-2 align-middle text-center">
                                  <textarea id={`info-electives-0-0`} value={studentInfo.electives[0]} onChange={(e) => handleInfoChange('electives', 0, null, e.target.value)} onKeyDown={(e) => handleInfoKeyDown(e, 'electives', 0, null)} onInput={autoResize} onFocus={handleFocus} onBlur={(e) => handleBlur(e, null, null, true, null, null)} className="w-full h-full bg-transparent resize-none outline-none text-center font-bold text-slate-700 auto-resize break-words whitespace-pre-wrap min-h-[80px]" style={{ fontSize: `${fontSize}px` }} />
                                </td>
                              )}
                              {rIdx === 0 && (
                                <td rowSpan={studentInfo.websites.length} className="border border-slate-300 p-2 align-middle text-center">
                                  <textarea id={`info-electives-0-1`} value={studentInfo.electives[1]} onChange={(e) => handleInfoChange('electives', 1, null, e.target.value)} onKeyDown={(e) => handleInfoKeyDown(e, 'electives', 1, null)} onInput={autoResize} onFocus={handleFocus} onBlur={(e) => handleBlur(e, null, null, true, null, null)} className="w-full h-full bg-transparent resize-none outline-none text-center font-bold text-slate-700 auto-resize break-words whitespace-pre-wrap min-h-[80px]" style={{ fontSize: `${fontSize}px` }} />
                                </td>
                              )}
                            </tr>
                          ))}
                    ...
                    <div className="flex flex-col xl:flex-row gap-4 w-full mt-2">
                      ...
                      <div className="flex-1 w-full overflow-x-auto custom-scrollbar">
                        <div className="bg-slate-100 border border-slate-300 p-2 font-black text-center mb-0 border-b-0 rounded-t-xl">
                          <textarea id={`info-midHeaders-0-0`} value={studentInfo.midHeaders[0]} onChange={(e) => handleInfoChange('midHeaders', 0, null, e.target.value)} onKeyDown={(e) => handleInfoKeyDown(e, 'midHeaders', 0, null)} onInput={autoResize} onFocus={handleFocus} onBlur={(e) => handleBlur(e, null, null, true, null, null)} className="w-full bg-transparent resize-none outline-none text-center font-black auto-resize" style={{ fontSize: `${Math.max(12, fontSize + 2)}px` }} rows={1} />
                        </div>
                        <table className="w-full min-w-[600px] border-collapse text-center table-fixed bg-white">
                          <tbody>
                            {studentInfo.schoolGrades.map((row, rIdx) => (
                              <tr key={`sg-${rIdx}`} className="hover:bg-indigo-50/30 transition-colors group">
                                {row.map((cellVal, cIdx) => (
                                  <td key={`sg-${rIdx}-${cIdx}`} className={`border border-slate-300 p-1 align-middle text-center ${row.join('').trim() === '' ? 'bg-red-50/30' : ''}`}>
                                    <textarea id={`info-schoolGrades-${rIdx}-${cIdx}`} value={cellVal} onChange={(e) => handleInfoChange('schoolGrades', rIdx, cIdx, e.target.value)} onKeyDown={(e) => handleInfoKeyDown(e, 'schoolGrades', rIdx, cIdx)} onInput={autoResize} onFocus={handleFocus} onBlur={(e) => handleBlur(e, null, null, true, null, null)} className={`w-full h-full bg-transparent resize-none outline-none text-center font-bold text-slate-700 auto-resize break-words whitespace-pre-wrap`} style={{ fontSize: `${fontSize}px` }} rows={1} />
                                  </td>
                                ))}
                              </tr>
                            ))}
                      ...
                      <div className="flex-1 w-full overflow-x-auto custom-scrollbar">
                        <div className="bg-slate-100 border border-slate-300 p-2 font-black text-center mb-0 border-b-0 rounded-t-xl">
                          <textarea id={`info-midHeaders-0-1`} value={studentInfo.midHeaders[1]} onChange={(e) => handleInfoChange('midHeaders', 1, null, e.target.value)} onKeyDown={(e) => handleInfoKeyDown(e, 'midHeaders', 1, null)} onInput={autoResize} onFocus={handleFocus} onBlur={(e) => handleBlur(e, null, null, true, null, null)} className="w-full bg-transparent resize-none outline-none text-center font-black auto-resize" style={{ fontSize: `${Math.max(12, fontSize + 2)}px` }} rows={1} />
                        </div>
                        <table className="w-full min-w-[500px] border-collapse text-center table-fixed bg-white">
                          <tbody>
                            {studentInfo.mockGrades.map((row, rIdx) => (
                              <tr key={`mg-${rIdx}`} className="hover:bg-indigo-50/30 transition-colors group">
                                {row.map((cellVal, cIdx) => (
                                  <td key={`mg-${rIdx}-${cIdx}`} className={`border border-slate-300 p-1 align-middle text-center ${row.join('').trim() === '' ? 'bg-red-50/30' : ''}`}>
                                    <textarea id={`info-mockGrades-${rIdx}-${cIdx}`} value={cellVal} onChange={(e) => handleInfoChange('mockGrades', rIdx, cIdx, e.target.value)} onKeyDown={(e) => handleInfoKeyDown(e, 'mockGrades', rIdx, cIdx)} onInput={autoResize} onFocus={handleFocus} onBlur={(e) => handleBlur(e, null, null, true, null, null)} className="w-full h-full bg-transparent resize-none outline-none text-center font-bold text-slate-700 auto-resize break-words whitespace-pre-wrap" style={{ fontSize: `${fontSize}px` }} rows={1} />
                                  </td>
                                ))}
                              </tr>
                            ))}
