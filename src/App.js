      ...
      >
        <div className="w-full h-full flex flex-col justify-center items-center p-0 text-center min-h-[30px] relative group/cell">
          ...
          <textarea
            ...
            className={`absolute inset-0 w-full h-full bg-transparent resize-none outline-none p-1 text-center font-bold text-slate-800 rounded shadow-sm overflow-hidden align-middle auto-resize break-words whitespace-pre-wrap ${isActiveThis && !isEditingThis ? 'select-none' : ''}`}
          />
          {!isEditingThis && (
            <div className="w-full h-full flex flex-col gap-1 justify-center items-center px-1 py-1 min-h-[30px] relative z-10 pointer-events-none">
              {val.trim() === '' ? (
                      ...
                      <table className="w-full border-collapse text-center table-fixed min-w-[700px]">
                        <thead>
                          <tr className="bg-slate-50 text-slate-700">
                            {[0, 1, 2].map(i => renderInfoCell('topHeaders', 0, i, studentInfo.topHeaders[i], { tag: 'th', extraClass: 'w-[10%]', extraStyle: { fontSize: `${Math.max(10, fontSize)}px`, fontWeight: 900 } }))}
                            {renderInfoCell('topHeaders', 0, 3, studentInfo.topHeaders[3], { tag: 'th', extraClass: 'w-[30%]', extraStyle: { fontSize: `${Math.max(10, fontSize)}px`, fontWeight: 900 } })}
                            {renderInfoCell('topHeaders', 0, 4, studentInfo.topHeaders[4], { tag: 'th', extraClass: 'w-[40%]', extraStyle: { fontSize: `${Math.max(10, fontSize)}px`, fontWeight: 900 } })}
                          </tr>
                        </thead>
                        <tbody>
                          {studentInfo.websites.map((row, rIdx) => (
                            <tr key={rIdx} className="hover:bg-slate-50 transition-colors">
                              {[0, 1, 2].map(cIdx => renderInfoCell('websites', rIdx, cIdx, row[cIdx], { extraStyle: { fontSize: `${fontSize}px`, fontWeight: 700 } }))}
                              
                              {rIdx === 0 && renderInfoCell('electives', 0, 0, studentInfo.electives[0], { rowSpan: studentInfo.websites.length, extraStyle: { fontSize: `${fontSize}px`, fontWeight: 700, minHeight: '80px' } })}
                              {rIdx === 0 && renderInfoCell('electives', 0, 1, studentInfo.electives[1], { rowSpan: studentInfo.websites.length, extraStyle: { fontSize: `${fontSize}px`, fontWeight: 700, minHeight: '80px' } })}
                            </tr>
                          ))}
                        </tbody>
                      </table>
                    ...
                    <div className="flex flex-col xl:flex-row gap-4 w-full mt-2">
                      ...
                      <div className="flex-1 w-full overflow-x-auto custom-scrollbar">
                        <div className="bg-slate-100 border border-slate-300 p-0 font-black text-center mb-0 border-b-0 rounded-t-xl overflow-hidden shadow-sm">
                          {renderInfoCell('midHeaders', 0, null, studentInfo.midHeaders[0], { tag: 'div', extraClass: 'w-full bg-slate-100 !border-none', extraStyle: { fontSize: `${Math.max(12, fontSize + 2)}px`, fontWeight: 900 } })}
                        </div>
                        <table className="w-full min-w-[600px] border-collapse text-center table-fixed bg-white shadow-sm rounded-b-xl overflow-hidden">
                          <tbody>
                            {studentInfo.schoolGrades.map((row, rIdx) => (
                              <tr key={`sg-${rIdx}`} className="hover:bg-indigo-50/30 transition-colors group/row relative">
                                {row.map((cellVal, cIdx) => renderInfoCell('schoolGrades', rIdx, cIdx, cellVal, { 
                                  ...
                                  extraStyle: { fontSize: `${fontSize}px`, fontWeight: 700 },
                      ...
                      <div className="flex-1 w-full overflow-x-auto custom-scrollbar">
                        <div className="bg-slate-100 border border-slate-300 p-0 font-black text-center mb-0 border-b-0 rounded-t-xl overflow-hidden shadow-sm">
                          {renderInfoCell('midHeaders', 1, null, studentInfo.midHeaders[1], { tag: 'div', extraClass: 'w-full bg-slate-100 !border-none', extraStyle: { fontSize: `${Math.max(12, fontSize + 2)}px`, fontWeight: 900 } })}
                        </div>
                        <table className="w-full min-w-[500px] border-collapse text-center table-fixed bg-white shadow-sm rounded-b-xl overflow-hidden">
                          <tbody>
