...
plans 배열은 무조건 12개의 문자열로 구성. 요청되지 않은 달은 빈 문자열("")로 두세요.`,
  ...
  // 💡 [방향 2] 뷰포트에 맞게 텍스트 사이즈 자동 조절 & 줄바꿈 허용 (모바일 전용)
  const getSmartMobileStyle = (text, align = 'center') => {
    const minSize = 6.5; // 너무 작아지지 않도록 하한선 지정 (방향 1 혼합)
    const maxSize = 8.5;
    if (!text) return { fontSize: `${maxSize}px`, textAlign: align, display: 'block', wordBreak: 'break-all', whiteSpace: 'pre-wrap', width: '100%' };
    ...
    if (maxLength === 0) return { fontSize: `${maxSize}px`, textAlign: align, display: 'block', wordBreak: 'break-all', whiteSpace: 'pre-wrap', width: '100%' };
    
    let idealSize = 32 / maxLength;
    if (idealSize > maxSize) idealSize = maxSize;
  ...
  const getSmartMobileStyleMonthly = (text, align = 'center') => {
    const minSize = 6; // 너무 작아지지 않도록 하한선 지정
    const maxSize = 7.5; 
    if (!text) return { fontSize: `${maxSize}px`, textAlign: align, display: 'block', wordBreak: 'break-all', whiteSpace: 'pre-wrap', width: '100%' };
          ...
          {printRows.map(row => (
            <tr key={row.id} className="h-[1%]">
              ...
              {printDays.map(day => {
                ...
                // 💡 [모바일] 줄바꿈 방지 해제, 계산된 폰트 크기 적용
                const lines = text.split('\n').filter(l => l !== '');
                const smartStyle = getSmartMobileStyle(text, 'center');
                    ...
                    <div className="w-full h-full flex flex-col items-center justify-center overflow-hidden max-h-full gap-[1px]">
                      {lines.map((line, lIdx) => (
                        <div key={lIdx} className="w-full flex items-center justify-center">
                          <span className="font-bold text-slate-800 text-center tracking-tighter" style={{ ...smartStyle }}>
                             {line}
                          </span>
                        </div>
                      ))}
                                                   ...
                                                   <td key={d.full} className="border border-slate-200 p-[1px] align-middle overflow-hidden bg-slate-50/30">
                                                     <div className="w-full h-full flex flex-col items-center justify-center overflow-hidden gap-[1px]">
                                                       {noteLines.map((line, lIdx) => (
                                                         <div key={lIdx} className="w-full flex items-center justify-center">
                                                           <span className="font-bold text-slate-700 block text-center tracking-tighter" style={{ ...noteStyle }}>
                                                             {line}
                                                           </span>
                                                         </div>
                                                       ))}
                                                   ...
                                                   {isFirst && <td rowSpan={rowSpan} className="border border-slate-200 bg-slate-50/50 align-middle overflow-hidden p-[1px]">
                                                      <div className="w-full h-full flex flex-col items-center justify-center overflow-hidden gap-[1px]">
                                                         {subLines.map((line, lIdx) => (
                                                            <div key={lIdx} className="w-full flex items-center justify-center">
                                                               <span className="font-black text-slate-800 block text-center tracking-tighter" style={{ ...subStyle }}>{line}</span>
                                                            </div>
                                                         ))}
                                                      </div>
                                                   </td>}
                                                     ...
                                                     <div className="w-full h-full flex flex-col items-center justify-center overflow-hidden gap-[1px]">
                                                       {tbLines.map((line, idx) => (
                                                         <div key={idx} className="w-full flex items-center justify-center">
                                                           <span className="font-bold text-slate-700 block text-center tracking-tighter" style={{ ...tbStyle }}>
                                                             {line}
                                                           </span>
                                                         </div>
                                                       ))}
                                                               ...
                                                               return (
                                                                  <div key={lIdx} className="flex justify-between items-center gap-[1px] text-left w-full overflow-hidden leading-[1.15] mb-[1.5px]">
                                                                     <div className="flex-1 min-w-0 flex items-center justify-start">
                                                                       <span className="font-bold text-slate-800 tracking-tighter text-left block" style={{ ...cellStyle }}>
                                                                          {line}
                                                                       </span>
                                                                     </div>
                                                                     {isChecked ? (
                                                ...
                                                <tr key={`status-m-${sub}-${tbIdx}`} className="bg-white">
                                                   {isFirst && <td rowSpan={rowSpan} className="border border-slate-200 bg-slate-50/50 align-middle overflow-hidden p-[1px]">
                                                       <div className="w-full h-full flex flex-col items-center justify-center overflow-hidden gap-[1px]">
                                                         {subLines.map((line, lIdx) => (
                                                           <div key={lIdx} className="w-full flex items-center justify-center">
                                                             <span className="font-black text-slate-800 block text-center tracking-tighter" style={{ ...subStyleInfo }}>{line}</span>
                                                           </div>
                                                         ))}
                                                       </div>
                                                   </td>}
                                                   <td className="border border-slate-200 align-middle overflow-hidden p-[1px]">
                                                       <div className="w-full h-full flex flex-col items-center justify-center overflow-hidden gap-[1px]">
                                                         {tbLines.map((line, idx) => (
                                                           <div key={idx} className="w-full flex items-center justify-center">
                                                             <span className="font-bold text-slate-700 block text-center tracking-tighter" style={{ ...tbStyleInfo }}>{line}</span>
                                                           </div>
                                                         ))}
                                                       </div>
                                                   </td>
                                                   <td className="border border-slate-200 bg-slate-50/5 align-middle overflow-hidden p-[1px]">
                                                       <div className="w-full h-full flex flex-col items-center justify-center overflow-hidden gap-[1px]">
                                                         {firstLines.map((line, idx) => (
                                                           <div key={idx} className="w-full flex items-center justify-center">
                                                             <span className="font-black text-indigo-700 block text-center tracking-tighter" style={{ ...firstStyleInfo }}>{line}</span>
                                                           </div>
                                                         ))}
                                                       </div>
                                                   </td>
                                                   <td className="border border-slate-200 bg-slate-50/5 align-middle overflow-hidden p-[1px]">
                                                       <div className="w-full h-full flex flex-col items-center justify-center overflow-hidden gap-[1px]">
                                                         {lastLines.map((line, idx) => (
                                                           <div key={idx} className="w-full flex items-center justify-center">
                                                             <span className="font-black text-rose-700 block text-center tracking-tighter" style={{ ...lastStyleInfo }}>{line}</span>
                                                           </div>
                                                         ))}
                                                       </div>
                                                   </td>
